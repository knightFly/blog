...
__author__ = 'knightFly'

import asyncio,inspect,os,logging,functools,aiohttp
import apis
...
class RequestHandler(object):
    ...
    def __call__(self, request):
        logging.info('********************')
        logging.info(self._has_request_args)
        kw = None
        if self._has_var_kw_args or self._has_named_kw_args or self._required_kw_args:
            if request.method == 'POST':
                ...
                if ct.startswith('application/json'):
                    ...
                    kw = params
                elif ct.startswith('application/x-www-form-urlencoded') and ct.startswith('multipart/form-data'):
        ...
        if kw is None:
            ...
            for k, v in request.match_info.items():
                ...
                kw[k] = v
        if has_request_args(self.__func):
            logging.info('*******************')
            kw['request'] = request
...
def add_route(app,fn):
    ...
    logging.info('add route %s %s ==> %s(%s)' % (method,path,fn.__name__,inspect.signature(fn).parameters.keys()))
